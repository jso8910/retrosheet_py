...
class Play:
    ...
    def parse_play(self):
        ...
        modifiers: list[str] = parts[0].replace(
            '-', '').replace('+', '').split('/')[1:] if len(parts[0].split('/')) > 1 else []
        if modifiers:
        ...
        def parse_fielders(fielder_string: str) -> list[PlayFlags | int]:
            ...
            for idx, char in enumerate(fielder_string):
                if char.isdigit() and not string_temp:
                    ...
                    print(
                        fielder_string, "THERE IS INTERFERENCE (n/INT) HERE, THIS MIGHT BE INTERESTING")
        ...
        def proc_modifiers():
            ...
            for modifier in modifiers:
                ...
                if modifier.startswith(("G", "L", "P", "F")) and modifier[1].isdigit():
                    ...
                    play_enum |= PlayFlags.THM | PlayFlags[bases[modifier[2]] + "_START"]
...
class Event:
    def __init__(self, event_string: str):
        ...
        self.count_of_play = (list(self.event_string.split(',')[4])[0], list(self.event_string.split(
            ',')[4])[1]) if self.event_string.split(',')[4] != "??" else (None, None)
        self.pitches = []
